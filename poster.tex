...
% ---------- Title block (logos flanking the title) ----------
\title{\parbox{\linewidth}{\centering
  \begin{minipage}[c]{0.15\linewidth}\centering
    \includegraphics[width=\linewidth]{logos/kdu_crest.png}
  \end{minipage}\hfill
  ...
  \end{minipage}\hfill
  \begin{minipage}[c]{0.15\linewidth}\centering
    \includegraphics[width=\linewidth]{logos/kdu_global.png}
  \end{minipage}
}}
...
\institute{Supervisor: \textbf{Prof.\ Dr.\ Jebran Khan} \\[4pt]
           Kyungdong University Global, South Korea \\[2pt]
...
\begin{tikzpicture}[
  ...
  outnode/.style={rectangle, draw=pTeal, fill=pTeal!15, line width=1pt,
  ...
  \node[outnode, below left=1.2cm and -0.3cm of rt] (tx) {Text / Table};
  \node[agent, below right=1.2cm and -0.3cm of rt] (pd) {\textbf{Phase B: Pandas Agent}\\ \small Plotly\,/\,Matplotlib code};
  \node[outnode, below=of pd]       (ch)    {\textbf{Sandboxed \texttt{exec()}} chart};
...
\begin{tikzpicture}[
  ...
  every node/.style={font=\small},
  mystep/.style={rectangle, rounded corners=4pt, line width=1pt,
...
\node[mystep] (a) {Synthetic\\ Dataset\\ \scriptsize 5{,}000};
\node[mystep, right=of a] (b) {Pre-\\process\\ \scriptsize scale+OHE};
\node[mystep, right=of b] (c) {Train/\\Test\\ \scriptsize 80/20};
\node[mystep, right=of c] (d) {5-fold\\ CV\\ \scriptsize LR/RF/GBM};
\node[mystep, right=of d] (e) {Grid\\ Search\\ \scriptsize tune};
\node[mystep, right=of e] (f) {Evaluate\\ \scriptsize ROC/PR/CM};
...
\addplot+[draw=pBlue, fill=pBlueLight!50, line width=1pt] coordinates {
